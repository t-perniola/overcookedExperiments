...
class MAPPO:
    ...
    def dynamic_layout_switching(self):
        ...
        if self.phase == 1 and mean(cramped, N = 5) >= 50:
            print("\n🎓 Phase 2 – Interleaved layouts")
            self.phase = 2
        if self.phase == 2 and mean(cramped, N = 5) >= 50 and mean(asym, N = 5) >= 20 and mean(bottleneck, N = 5) >= 20:
    ...
    def rollout(self, episodes, rew_shaping):
        buffer = Buffer()           
        ...
        for _ in range(episodes):
            ...
            for _ in range(self.horizon):
                ...
                # Apply decaying
                team_reward = self.reward_shaping(n_rollout=self.num_rollouts,
                                                  rew=team_reward,
